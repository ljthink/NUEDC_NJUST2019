...
static inline int16_t speed_base(void)
{
  if (status.sensor == Inductance)
    return 80; /* 电感模式 */
  else if(adc_roaddata.status > CircleConditon ) /* 环内减速 */
    return 80;
  else
    return 170;
}
...
static inline int16_t speed_differential(int16_t base_speed)
{
  ...
  if (R<-0.006)/*左小弯*/
  {
    return (int16_t)(base_speed * 7.5*8.167*R);
  }
  else if(R>0.006)/*右小弯*/
  {
    return (int16_t)(base_speed * 7.5*7.667*R);
  }

  else if ( R>=-0.006 && R<-0.004)/*左大弯*/
  {
    return (int16_t)(base_speed * 7.5*6.5*R);
  }
  else if (R>0.004 && R<=0.006)/*右大弯*/
  {
    return (int16_t)(base_speed * 7.5*6.167*R);
  }
  else 
    return 0;
}
