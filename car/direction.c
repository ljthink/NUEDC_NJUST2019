...
static void car_direction_control_arcman(void)
{
  ...
  */
  arc_err = 980*R;
  ud = 31*0.5*(arc_err - arc_err1) + 0.5*ud1;
  
  if (R<=0.004 && R>=-0.004)  /*直线*/
    servo_pwm = (uint16_t)(SERVO_MID + 6*arc_err +ud);
  
  else if(R>0.004 && R<0.006) /*右大弯*/
    servo_pwm = (uint16_t)(SERVO_MID + 37*arc_err - 121.52 + ud); 
  else if(R>-0.006 && R<-0.004)  /*左大弯*/
    servo_pwm = (uint16_t)(SERVO_MID + 38*arc_err + 129.36 + ud);
  
  else if(R>0.006) /*右小弯*/
    servo_pwm = (uint16_t)(SERVO_MID + 46*arc_err - 174.44 + ud);
  else if(R<-0.006)/*左小弯*/
    servo_pwm = (uint16_t)(SERVO_MID + 49*arc_err + 188.16 + ud);
...
static void car_direction_control_circle(void)
{
  ...
  /* 环内 */
  if(adc_roaddata.status == RightCircleWaitIn && ENC_GetPositionValue(ENC2)>20000)
  {
    adc_roaddata.status = RightCircleRun;
    return;
  }
  
  if(adc_roaddata.status == RightCircleWaitIn && ENC_GetPositionValue(ENC2)>4500)
  {
    return;
  }  
  ...
  if(adc_roaddata.status == RightCircleWaitIn && ENC_GetPositionValue(ENC2)>3300)
  {
   ...
   /* 出环完成 */
  if ( adc_roaddata.status == RightCircleWaitOut && ENC_GetPositionValue(ENC2)>5000 )
  {
